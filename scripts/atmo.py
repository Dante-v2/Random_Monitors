...
class ATMO():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'atmo/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "atmo/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running ATMO | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running ATMO | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchnew(self):
        ...
        while True:
            try:
                ...
                prod = self.s.get(
                    'https://atmotube.de/products.json',
                    headers = headers,
                    timeout=125    
                )
                if prod.status_code == 200:
                    ...
                    for i in x['products']:
                        if i['title'] == self.link:
                            if i['variants'][0]['available'] == True:
                                self.success('Product instock!')
                                webhook = DiscordWebhook(url='https://discord.com/api/webhooks/905203339422228560/3A3sOTKl_YLJCtS2N6yv4SDXSRZd4VHbQGirWzDT0MMTzW1yBArPwE6Wcm50DPbDwLNi', content = "")
