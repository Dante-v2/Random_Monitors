...
class SENSE():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'sense/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "sense/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [ATMO] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running SENSE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running SENSE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchnew(self):
        ...
        while True:
            try:
                ...
                prod = self.s.get(
                    'https://sensecap.heliumhotshop.eu/products.json',
                    headers = headers,
                    timeout=125    
                )
                if prod.status_code == 200:
                    ...
                    for i in x['products']:
                        if i['variants'][0]['available'] == True:
                            self.success('Product instock!')
                            webhook = DiscordWebhook(url='https://discord.com/api/webhooks/905384315859394570/IoBYUeVvFQdNvdowQADTFbsrGKG-5Bf79XvDtXV9Mh4qGR3l7FR-ApKNA5Vg-Pz_duSQ', content = "")
                            embed = DiscordEmbed(title=self.link, url = f'https://atmotube.de/products/{i["handle"]}', description = '`Product instock!`', color = 0x715aff)
                            embed.add_embed_field(name='**Site**', value = f'[Atmotube](https://atmotube.de/)', inline = True)
                            embed.add_embed_field(name='**Price**', value = i['variants'][0]['price'], inline = True)
                            embed.add_embed_field(name='**ATC**', value = f"[SINGLE](https://atmotube.de/cart/{i['variants'][0]['id']}:1)", inline = False)
                            embed.set_thumbnail(url=i['images'][0]['src'])
                            embed.set_footer(text = f" Monitor - Atmotube", icon_url = "")
                    ...
                    self.warn('Password page up, retring...')
                    time.sleep(self.delay)
                    continue
