...
class MICROMANIA():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'micromania/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "micromania/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [MICROMANIA] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running MICROMANIA | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running MICROMANIA | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
        ...
        while True:
            try:
                ...
                if prod.status_code == 200:
                    if 'data-inStock="0.0"' in prod.text:
                        self.warn('Product oos, retrying...')
                        time.sleep(self.delay)
                        continue
                    else:
                        z = prod.text.split('type="image/png" href="')[1].split('" sizes="196x196')[0]
                        self.success('Product instock!')
                        webhook = DiscordWebhook(url='https://discord.com/api/webhooks/905125510122733700/pdsuk0NpXnhpuBepR67eGbnTHVTXQkir4WhZMApO2AYDfWZA3toy3MO-H-y7JyKoRaAI', content = "")
                        embed = DiscordEmbed(title=prod.text.split('<title>')[1].split('</title>')[0], url = self.link, description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Micromania](https://www.micromania.fr/)', inline = True)
                        embed.add_embed_field(name='**Price**', value = prod.text.split('itemprop="price" content="')[1].split('"')[0], inline = True)
                        embed.set_thumbnail(url=prod.text.split('<img src="')[1].split('"')[0])
                        embed.set_footer(text = f" Monitor - Micromania", icon_url = "")
