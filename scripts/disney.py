...
class DISNEY():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'disney/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "disney/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [DISNEY] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running DISNEY | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running DISNEY | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchnew(self):
        ...
        if self.country == 'IT':
            prodvar = 'https://www.shopdisney.it/on/demandware.store/Sites-shopDisneyIT-Site/it_IT/Product-Variation'
            flag = ':flag_it:'
            sito = 'https://www.shopdisney.it/'
        elif self.country == 'DE':
            prodvar = 'https://www.shopdisney.de/on/demandware.store/Sites-shopDisneyDE-Site/de_DE/Product-Variation'
            flag = ':flag_de:'
            sito = 'https://www.shopdisney.de/'
        elif self.country == 'EU':
            prodvar = f'https://www.shopdisney.eu/on/demandware.store/Sites-shopDisneyEU-Site/en_IE/Product-Variation'
            flag = ':flag_eu:'
            sito = 'https://www.shopdisney.eu/'
        elif self.country == 'FR':
            prodvar = 'https://www.shopdisney.fr/on/demandware.store/Sites-shopDisneyFR-Site/fr_FR/Product-Variation'
            flag = ':flag_fr:'
            sito = 'https://www.shopdisney.fr/'
        elif self.country == 'UK':
            prodvar = 'https://www.shopdisney.co.uk/on/demandware.store/Sites-shopDisneyEN-Site/en_GB/Product-Variation'
            flag = ':flag_gb:'
            sito = 'https://www.shopdisney.co.uk/'
        elif self.country == 'ES':
            prodvar = 'https://www.shopdisney.es/on/demandware.store/Sites-shopDisneyES-Site/es_ES/Product-Variation'
            flag = ':flag_es:'
            sito = 'https://www.shopdisney.es/'
        else:
        ...
        while True:
            try:
                ...
                prod = self.s.get(
                    f'{prodvar}?dwopt_{self.link}_personalization=none&pid={self.link}&quantity=1&quickview=false',
                    headers = headers,
                    timeout=125    
                )
                if prod.status_code == 200:
                    x = json.loads(prod.text)
                    if x['product']['available'] == True:
                        self.success('Product instock!')
                        webhook = DiscordWebhook(url='https://discord.com/api/webhooks/905058599640911912/p_3RUj_YEutGhk8GGE2XKXmAKy4td955vm-1ErKfmmFIU2LB_khmcYPFjrGteyzvW8Mh', content = "")
                        embed = DiscordEmbed(title=x['product']['productName'], url = f'{sito}goat-{self.link}.html', description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Disney {flag}]({sito})', inline = True)
                        embed.add_embed_field(name='**Price**', value = x['product']['price']['selling']['formatted'], inline = True)
                        embed.set_thumbnail(url=x['product']['images']['large'][0]['url'])
                        embed.set_footer(text = f" Monitor - Disney", icon_url = "")
