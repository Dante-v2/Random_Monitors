...
class HYPEDCARTEL():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'hypedcartel/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "hypedcartel/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [HYPEDCARTEL] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running HYPEDCARTEL | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running HYPEDCARTEL | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def pidmonitor(self):
        ...
        while True:
            try:
                r = self.s.get(
                    f'https://hypedcartel.com/products.json',
                ...
                if r.status_code == 200:
                    ...
                    variant = []
                    size = []
                    with open('hypedcartel/data.json', 'r') as f:
                        data = json.load(f)
                    for i in r_json['products']:
                        if i['title'] not in data.keys():
                            ...
                            variant.append(i['variants'][0]['id'])
                            size.append(i['variants'][0]['title'])
                    if not variant:
                        self.warn('No products found, monitoring...')
                        time.sleep(self.delay)
                        continue
                    tot = zip(title,img,price,variant,size)
                    ...
                    for z in title:
                        if z not in data.keys():
                            ...
                            for b in self.connect:
                                if z == b[0]:
                                    data[b[0]] = b[3]
                                    with open('hypedcartel/data.json', 'w') as f:
                                        json.dump(data, f, indent=4)
                                    f.close()
                                    addre = 'https://hypedcartel.com/checkout'
                                    webhook = DiscordWebhook(url=self.webhook_url, content = "")
                                    embed = DiscordEmbed(title=b[0], color = 0x715aff)
                                    embed.add_embed_field(name='**Price**', value = b[2], inline = True)
                                    embed.add_embed_field(name='**Site**', value = f'[Hypedcartel](https://hypedcartel.com/)', inline = True)
                                    embed.add_embed_field(name="**Sizes**", value=f'[{b[4]}](https://hypedcartel.com/cart/{b[3]}:1)', inline=False)
                                    embed.add_embed_field(name='**Checkout Link**', value = f'[Checkout]({addre})', inline = False)
                                    embed.set_thumbnail(url=b[1])
                                    embed.set_footer(text = f"Dante Monitors - Hypedcartel", icon_url = "")
