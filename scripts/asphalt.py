...
class ASPHALT():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'asphalt/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "asphalt/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [ASPHALT] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running ASPHALT | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running ASPHALT | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def pidmonitor(self):
        ...
        while True:
            try:
                r = self.s.get(
                    'https://www.asphaltgold.com/products.json',
                ...
                if r.status_code == 200:
                    ...
                    size = []
                    handle = []
                    with open('asphalt/data.json', 'r') as f:
                        data = json.load(f)
                    for i in r_json['products']:
                        if i['title'] not in data.keys():
                            if 'Dunk' in i['title'] or 'Yeezy' in i['title'] or 'Jordan 1' in i['title'] or 'Jordan 4' in i['title']:
                    ...
                    for z in handle:
                        if z not in data.keys():
                            ...
                            for b in self.connect:
                                if z == b[5]:
                                    data[b[0]] = b[5]
                                    with open('asphalt/data.json', 'w') as f:
                                        json.dump(data, f, indent=4)
                                    f.close()
                                    addre = 'https://www.asphaltgold.com/checkout'
                                    webhook = DiscordWebhook(url=self.webhook_url, content = "")
                                    embed = DiscordEmbed(title=b[0], url=f'https://www.asphaltgold.com/products/{b[5]}', color = 0x715aff)
                                    embed.add_embed_field(name='**Price**', value = b[2], inline = True)
                                    embed.add_embed_field(name='**Site**', value = f'[Asphaltgold](https://www.asphaltgold.com/)', inline = True)
                                    try:
                                        emb = []
                                        for k in size:
                                            for i in self.connect:
                                                if k == i[4]:
                                                    emb.append(f'[{i[4]}](https://www.asphaltgold.com/cart/{i[3]}:1)')
                                    ...
                                    embed.set_footer(text = f" Monitors - Asphaltgold", icon_url = "")
