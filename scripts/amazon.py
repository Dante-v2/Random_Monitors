...
class AMAZON():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'amazon/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "amazon/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [AMAZON] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running AMAZON | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running AMAZON | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchnew(self):
        ...
        if self.country == 'IT':
            prodvar = 'https://www.amazon.it/-/en/dp/'
            flag = ':flag_it:'
            sito = 'https://www.amazon.it/'
            reflink = 'goat078-21'
        elif self.country == 'DE':
            prodvar = 'https://www.amazon.de/-/en/dp/'
            flag = ':flag_de:'
            sito = 'https://www.amazon.de/'
            reflink = 'goat0a5-21'
        elif self.country == 'NL':
            prodvar = 'https://www.amazon.nl/-/en/dp/'
            flag = ':flag_de:'
            sito = 'https://www.amazon.nl/'
            reflink = 'goat00-21'
        elif self.country == 'FR':
            prodvar = 'https://www.amazon.fr/-/en/dp/'
            flag = ':flag_fr:'
            sito = 'https://www.amazon.fr/'
            reflink = 'goat0a-21'
        elif self.country == 'UK':
            prodvar = 'https://www.amazon.co.uk/-/en/dp/'
            flag = ':flag_gb:'
            sito = 'https://www.amazon.co.uk/'
            reflink = 'goat0a5-21'
        elif self.country == 'ES':
            prodvar = 'https://www.amazon.es/-/en/dp/'
            flag = ':flag_es:'
            sito = 'https://www.amazon.es/'
            reflink = 'goat09-21'
        else:
        ...
        while True:
            try:
                self.warn('Monitoring...')
                print(f'{prodvar}{self.link}')
                prod = self.s.get(
                    f'{prodvar}{self.link}',
                    headers = headers,
                    timeout=125    
                ...
                if prod.status_code == 200:
                    if 'name="offerListingID" value=""' in prod.text:
                        self.warn('Product oos, retrying...')
                        time.sleep(self.delay)
                        continue
                    else:
                        price = prod.text.split('data-a-color="price"><span class="a-offscreen">')[1].split('</span>')[0]
                        img = prod.text.split('data-old-hires="')[1].split('"')[0]
                        title = prod.text.split('name="title" content="')[1].split('"')[0]
                        try:
                            title = title.split(':')[0]
                        except:
                            pass
                        listingid = prod.text.split('name="offerListingID" value="')[1].split('"')[0]
                        atc = f'{sito}gp/aws/cart/add-res.html?Quantity.1=1&OfferListingId.1={listingid}&AssociateTag={reflink}'
                        self.success('Product instock!')
                        webhook = DiscordWebhook(url=self.webhook_url, content = "")
                        embed = DiscordEmbed(title=title, url = f'{prodvar}{self.link}', description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Amazon {flag}]({sito})', inline = True)
                        embed.add_embed_field(name='**Price**', value = price, inline = True)
                        embed.add_embed_field(name='**ATC**', value = f'[Click Here]({atc})', inline = False)
                        embed.set_thumbnail(url=img)
                        embed.set_footer(text = f" Monitor - Amazon", icon_url = "")
