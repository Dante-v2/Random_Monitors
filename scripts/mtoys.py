...
class MTOYS():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'mtoys/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "mtoys/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [MTOYS] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running MTOYS | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running MTOYS | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
            ...
            try:
                ...
                if r.status_code == 200:
                    if 'In den Warenkorb' in r.text:
                        ...
                        embed = DiscordEmbed(title=r.text.split('property="og:title" content="')[1].split('"')[0], url = self.link, description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Mtoys DE](https://www.mytoys.de/)', inline = True)
                        embed.add_embed_field(name='**Price**', value = r.text.split('currentPrice: "')[1].split('"')[0], inline = True)
                        embed.set_thumbnail(url=f'''https:{r.text.split('property="og:image" content="')[1].split('"')[0]}''')
                        embed.set_footer(text = f" Monitor - Mtoys DE", icon_url = "")
