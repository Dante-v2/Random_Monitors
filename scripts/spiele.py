...
class SPIELE():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'spiele/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "spiele/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [SPIELE] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running SPIELE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running SPIELE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchnew(self):
        ...
        while True:
            try:
                ...
                prod = self.s.get(
                    f'https://spielegrotte.de/index.php?suchwort={self.link}',
                    headers = headers,
                    timeout=125    
                )
                if prod.status_code == 200:
                    if 'PlayStation 5 leider komplett ausverkauft!' in prod.text:
                        ...
                        embed = DiscordEmbed(title='PS5', url = f'https://spielegrotte.de/index.php?suchwort={self.link}', description = '`PS5 Back instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Spielegrotte](https://spielegrotte.de/)', inline = True)
                        embed.set_footer(text = f" Monitor - Spielegrotte", icon_url = "")
