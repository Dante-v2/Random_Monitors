...
class OTTO():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'otto/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "otto/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [OTTO] [{self.mode}] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running OTTO | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running OTTO | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
        ...
        while True:
            try:
                ...
                if prod.status_code == 200:
                    if self.pid not in prod.url:
                        self.warn('Product oos, retrying...')
                        time.sleep(self.delay)
                        continue
                    else:
                        self.success('Product instock!')
                        title = prod.text.strip().split('title>')[1].split('</')[0]
                        image = prod.text.split('property="og:image" content="')[1].split('"')[0]
                        price = prod.text.split('formattedPriceAmount":"')[1].split('"')[0]
                        webhook = DiscordWebhook(url=self.webhook_url, content = "")
                        embed = DiscordEmbed(title=title, url = prod.url, description = '`Product Instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Otto](https://www.otto.de/)', inline = True)
                        embed.add_embed_field(name="**Price**", value=price, inline=True)
                        embed.set_thumbnail(url=image)
                        embed.set_footer(text = f" Monitor - Otto", icon_url = "")
    ...
    def searchnew(self):
        ...
        while True:
            try:
                ...
                if prod.status_code == 200:
                    if 'Vielen Dank an alle, die eine PlayStation 5 bestellt haben. Aktuell ist die PlayStation 5 auf otto.de leider ausverkauft.' not in prod.text and release == 0:
                        webhook = DiscordWebhook(url=self.webhook_url, content = "")
                        embed = DiscordEmbed(title=f'PS5 Drop Soon', url = 'https://www.otto.de/technik/gaming/playstation/ps5/', description = '`Be ready!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Otto](https://www.otto.de/)', inline = True)
                        embed.set_footer(text = f" Monitor - Otto", icon_url = "")
                        webhook.add_embed(embed)
                        webhook.execute()
                        release = release + 1
                        self.success('Release started!')
                    if 'Vielen Dank an alle, die eine PlayStation 5 bestellt haben. Aktuell ist die PlayStation 5 auf otto.de leider ausverkauft.' in prod.text and release == 1:
                        webhook = DiscordWebhook(url=self.webhook_url, content = "")
                        embed = DiscordEmbed(title=f'PS5 Release', url = 'https://www.otto.de/technik/gaming/playstation/ps5/', description = '`Drop is Over!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Otto](https://www.otto.de/)', inline = True)
                        embed.set_footer(text = f" Monitor - Otto", icon_url = "")
                        webhook.add_embed(embed)
                        webhook.execute()
                        release = 0
                        self.success('Release ended!')
                    soup = bs(prod.text, features='lxml')
                    items = soup.find('div',{'class':'promo_articlelist--gallery promo_articlelist--container'}).find_all('li',{'class':'promo_articlelist--article'})
                    images = []
                    titles = []
                    urls = []
                    pid = []
                    with open('otto/data.json', 'r') as f:
                        data = json.load(f)
                    for i in items:
                        if i['data-variation-id'] not in data.keys():
                            titles.append(i.find('a')['title'])
                            images.append(i.find('img')['src'])
                            urls.append(f"https://www.otto.de{i.find('a')['href']}")
                            pid.append(i['data-variation-id'])
                    if not pid:
                        self.warn('Nothing new, monitoring...')
                        time.sleep(self.delay)
                        continue
                    tot = zip(titles,images,urls,pid)
                    self.connect = list(tot)
                    with open('otto/data.json', 'r') as f:
                        data = json.load(f)
                    for z in self.connect:
                        if z[3] not in data.keys():
                            self.success('New product found!')
                            for b in self.connect:
                                if z[3] == b[3]:
                                    data[b[3]] = b[2]
                                    with open('otto/data.json', 'w') as f:
                                        json.dump(data, f, indent=4)
                                    f.close()
                                    link = b[2]
                                    sito = 'https://www.otto.de/'
                                    webhook = DiscordWebhook(url=self.webhook_url, content = "")
                                    embed = DiscordEmbed(title=f'{b[0]}', url = link, description = '`New Product Detected!`', color = 0x715aff)
                                    embed.add_embed_field(name='**Site**', value = f'[Otto]({sito})', inline = True)
                                    embed.add_embed_field(name='**Pid**', value = f'`{b[3]}`', inline = True)
