...
class WEHKAMP():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'wehkamp/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "wehkamp/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [WEHKAMP] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running WEHKAMP | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running WEHKAMP | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
        ...
        while True:
            try:
                ...
                if prod.status_code == 200:
                    if 'geef me een seintje' in prod.text:
                        self.warn('Product oos, retrying...')
                        time.sleep(self.delay)
                        continue
                    else:
                        self.success('Product instock!')
                        webhook = DiscordWebhook(url='https://discord.com/api/webhooks/905125555257626664/B0f2o_K-5vRhgYJCANpEd7Qs3brx2fhwO9t3m6Ce8bA0v68LNxI9jGCkb9_ct3BdN1LE', content = "")
                        embed = DiscordEmbed(title=prod.text.split('<title data-react-helmet="true">')[1].split('</title>')[0], url = self.link, description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Wehkamp NL](https://www.wehkamp.nl/)', inline = True)
                        embed.add_embed_field(name='**Price**', value = prod.text.split('price":"')[1].split('"')[0], inline = True)
                        embed.set_thumbnail(url=prod.text.split('og:image" content="')[1].split('"')[0])
                        embed.set_footer(text = f" Monitor - Wehkamp NL", icon_url = "")
