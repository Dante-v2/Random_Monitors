...
class LVR():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'lvr/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "lvr/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [LVR] [{self.mode}] [{self.link}] [{self.sesso}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running LVR | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running LVR | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchfe(self):
        ...
        f = self.s.get(
            'https://www.luisaviaroma.com/en-it/',
            headers = headers,
            timeout=125
        )
        print(f.status_code)
        while True:
            try:
                ...
                prod = self.s.get(
                    f'https://www.luisaviaroma.com/en-it/shop/women/search?lvrid=_gw_f&noredirect=1&search=dunk',
                    headers = headers,
                    timeout=125    
                )
                if prod.status_code == 200:
                    jsontext = prod.text.split('<script>window.__BODY_MODEL__ = ')[1].split(';window.hydratecounter')[0]
                    ...
                    urls = []
                    pid = []
                    with open('lvr/data.json', 'r') as f:
                        data = json.load(f)
                    for i in items:
                        if 'Sneakers' in i['Description'] or 'sneakers' in i['Description']:
                            if i['ItemCode'] not in data.keys():
                                titles.append(i['Description'])
                                images.append(f"https://images.lvrcdn.com/Big{i['Image']}")
                                sizes.append(i['SizeArray'])
                                urls.append(f"https://www.luisaviaroma.com/{i['Url']}")
                                pid.append(i['ItemCode'])
                    if not pid:
                        self.warn('Nothing new, monitoring...')
                        time.sleep(4)
                        continue
                    if not sizes:
                        self.warn('No sizes available, monitoring...')
                        time.sleep(4)
                        continue
                    tot = zip(titles,images,urls,sizes,pid)
                    ...
                    for z in self.connect:
                        if z[4] not in data.keys():
                            ...
                            for b in self.connect:
                                if z[4] == b[4]:
                                    data[b[4]] = b[0]
                                    with open('lvr/data.json', 'w') as f:
                                        json.dump(data, f, indent=4)
                                    f.close()
                                    link = b[2]
                                    addre = 'https://www.luisaviaroma.com/myarea/myCart.aspx?__s=MTg4NjYwMw#checkout'
                                    sito = 'https://www.luisaviaroma.com/'
