...
class BOL():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'bol/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "bol/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [BOL] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running BOL | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running BOL | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
        ...
        while True:
            try:
                ...
                if prod.status_code == 200:
                    if 'Niet leverbaar' in prod.text:
                        ...
                        embed = DiscordEmbed(title=prod.text.split('property="og:title" content="')[1].split('"')[0], url = self.link, description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Bol](https://www.bol.com/)', inline = True)
                        embed.add_embed_field(name='**Price**', value = prod.text.split('data-test="buy-block-sticky-cta-price">')[1].split('<')[0], inline = True)
                        embed.set_thumbnail(url=prod.text.split('property="og:image" content="')[1].split('">')[0])
                        embed.set_footer(text = f" Monitor - Bol", icon_url = "")
