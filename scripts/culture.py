...
class CULTURE():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'culture/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "culture/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [TOYS] [{self.link}] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running CULTURE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running CULTURE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
    ...
    def searchnew(self):
        ...
        while True:
            try:
                ...
                if "geo.captcha" in prod.text:
                    self.warn('Datadome found, solving...')
                    self.referer = prod.url
                    if "t':'fe" in prod.text:
                        self.t = prod.text.split("t':'")[1].split("'")[0]
                        self.initialcid = prod.text.split("cid':'")[1].split("'")[0]
                        self.hsh = prod.text.split("hsh':'")[1].split("'")[0]
                        self.sss = prod.text.split("s':")[1].split(",'")[0]
                        cid = []
                    ...
                    if "document.location.href = '" in prod.text:
                        l = prod.text.split("document.location.href = '")[1].split("';")[0]
                        print(f"https://www.cultura.com{l}")
                        m = self.s.get(f"https://www.cultura.com{l}",headers = headers)
                        print(m.status_code)
                        print(m.text)
                        continue
                    print(prod.text)
                    break
                    if 'En cours de' in prod.text:
                        ...
                        embed = DiscordEmbed(title=prod.text.split('<title>'[1].split('</title>')[0]), url = self.link, description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Cultura](https://www.cultura.com)', inline = True)
                        embed.add_embed_field(name='**Price**', value = prod.text.split('product_unitprice_ati"]="')[1].split('"')[0], inline = True)
                        embed.set_thumbnail(url=prod.text.split('id="main-image" href="')[1].split('"')[0])
                        embed.set_footer(text = f" Monitor - Cultura", icon_url = "")
    ...
    def solvedd(self):
        try:
            ...
            r = self.s.get(captchalink, headers=h)
            if r.status_code == 200:
                ...
                payload = {
                    ...
                    {
                        "type":"NoCaptchaTask",
                        "websiteURL":'https://www.cultura.com/',
                ...
                if "ready" in r.text:
                    ...
                    dd_url = 'https://geo.captcha-delivery.com/captcha/check?cid=' + self.cid + '&icid=' + self.initialcid +'&ccid=null&g-recaptcha-response=' + self.captcha +'&hash=' + self.hsh +'&ua=' + self.s.headers['User-Agent'] +'&referer=' + self.link +'&parent_url=' + 'https://www.cultura.com/' + '&x-forwarded-for=&captchaChallenge=' + '&s=' + str(self.sss)
                    r = self.s.get(dd_url, headers = h)
                    if r.status_code == 200:
                        jsondd = json.loads(r.text)
                        dd = jsondd['cookie']
                        dd = dd.split('datadome=')[1].split(';')[0]
                        self.cookie_obj = requests.cookies.create_cookie(domain=f".cultura.com",name='datadome',value=dd)
