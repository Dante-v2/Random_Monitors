...
class TOYS():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'toys/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "toys/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running TOYS | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running TOYS | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
        ...
        while True:
            try:
                ...
                if prod.status_code == 200:
                    ...
                    if x['@graph'][0]['offers'][0]['availability'] == 'http://schema.org/InStock':
                        ...
                        embed = DiscordEmbed(title=x['@graph'][0]['name'], url = self.link, description = '`Product instock!`', color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Toyscenter IT](https://www.toyscenter.it/)', inline = True)
                        embed.add_embed_field(name='**Price**', value = x['@graph'][0]['offers'][0]['price'], inline = True)
                        embed.set_thumbnail(url=x['@graph'][0]['image'])
                        embed.set_footer(text = f" Monitor - Toyscenter IT", icon_url = "")
