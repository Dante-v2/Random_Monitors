...
class HERE():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        ...
        self.logs_path = os.path.join(os.path.dirname(sys.argv[0]), 'here/exceptions.log')
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'here/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "here/proxies.txt")
            with open(f'{path}', 'r') as f:
        ...
        self.domain = "www.here-store.com"
        self.lang = "it_IT"
        self.site_region = "Sites-here-it-Site"
    ...
    def error(self, text):
        message = f'[TASK {self.threadID}] - [HERE] [{self.pidmonitor}] - {text}'
        error(message)
    ...
    def solvedd(self):
        try:
            captchalink = f"https://geo.captcha-delivery.com/captcha/?initialCid={self.initialcid}&hash={self.hsh}&t=fe&s={self.sss}&cid={self.cid}&referer={self.referer}"
            headers = {
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            ctypes.windll.kernel32.SetConsoleTitleW(
                f' Monitors {self.version} - Running HERE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}')
        else:
            sys.stdout.write(f'\x1b]2; Monitors {self.version} - Running HERE | 2cap Balance: {self.balance} | Carted: {carted} | Checkout: {checkoutnum} | Failed: {failed}\x07')
        ...
        try:
            ...
            if "geo.captcha" in r.text:
                ...
                if "t':'fe" in r.text:
                    ...
                    self.warn('Proxy banned, rotating...')
                    self.build_proxy()           
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if instockpids:
                        ...
                        sito = 'https://www.here-store.com/'
                        linknaz = f'https://www.here-store.com/-{instockpids[0][:-3]}.html'
                        addre = 'https://www.here-store.com/it/spedizione'
                        payment = 'https://www.here-store.com/it/fatturazione'
                        color = instockpids[0].split('_')[2]
                        phoenixqt = f'http://127.0.0.1:5005/phoenixqt?site=here&input={instockpids[0][:-3]}'
                        image = f'https://www.here-store.com/dw/image/v2/BCLG_PRD/on/demandware.static/-/Sites-awlab-master-catalog/default/dwe3c277df/images/large/{color}_0.jpg?sw=300'
                        webhook = DiscordWebhook(url=self.webhook_url, content = "")
                        embed = DiscordEmbed(title=instockpids[0][:-3], url = linknaz, color = 0x715aff)
                        embed.add_embed_field(name='**Site**', value = f'[Here Store]({sito})', inline = True)
                        emb = []
                        for z in instockpids:
                            size = z.split('_')[-1]
                            emb.append(f'[{size}](https://www.here-store.com/-{z}.html)')
                        ...
                        embed.add_embed_field(name='**Checkout Link**', value = f'[Shipping]({addre})\n[Payment]({payment})', inline = False)
                        embed.add_embed_field(name='**Quicktasks**', value = f'[Phoenix]({phoenixqt})', inline = True)
                        embed.set_thumbnail(url=image)
                        embed.set_footer(text = f" Monitor - Here Store", icon_url = "")
