...
class FOOTPATROL():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        ...
        self.logs_path = os.path.join(os.path.dirname(sys.argv[0]), 'footpatrol/exceptions.log')
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'footpatrol/proxies.txt')
            elif machineOS == "Windows":
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "footpatrol/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def search(self):
        ...
        while True:
            try:
                r = self.s.get(
                    'https://www.footpatrol.com/product/white-reebok-npc-ii/000281_footpatrolcom/stock/',
                    headers=head,
                    timeout=20
                )
                if r.status_code == 200:
                    soup = bs(r.text, features='lxml')
                    v = soup.find('div',{'id':'itemOptions'})
                    print(v)
                    break
